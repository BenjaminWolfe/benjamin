---
title: "R Markdown: Shrink & Tidy Your HTML Output with CDNs"
...
tags:
  ...
  - Reproducibility
subtitle: ''
summary: 'Ever tried sharing a standalone HTML report created in R Markdown? They can be positively HUGE.'
authors: []
lastmod: '2020-03-15T09:26:00-05:00'
featured: no
image:
  caption: ''
  focal_point: ''
...
In a moment we'll script and automate what we just did.
But in the end, this method still leaves you with a _folder_ of files to share.
That's a minor frustration in email-heavy settings.

The simple fix is to compress both the report and the report files
into a single zip folder.

![](35-zip-files.png)

In this case, the final zipped size was a meager 80 kilobytes:
less than a tenth the size of the HTML file we started with!

![](36-final-zipped-size.png)

You may still use R in a context
where stakeholders either can't or won't open a zip file
to click through on the single HTML file inside it.
Unfortunately, I don't have a solution for that. `r emo::ji("shrug")`
...
This will be particularly useful when we get to automating the script.
If the folder we use is a git repository, for example,
we can tell git to tidy up and shrink all its HTML files before every commit.
...
Now that we've written a script to "cdnify" all the HTML reports in a folder,
how can we automate the process?
Ideally we wouldn't have to run this script every time we render a report.

We can automate it using a feature called [git hooks][23].
_Git hooks_ are bash scripts that run after specific events in git;
you can write hooks that run automatically before a commit,
before you rebase, or before you push changes, for example.

In this case, we'll tell the bash script where to find our R script,
...
(If you're not yet using git, I highly recommend it.
There are some great resources out there to learn,
including Jenny Bryan & Jim Hester's [Happy Git and GitHub for the useR][24].)

Githooks live in a hidden directory.
...
or Command-Shift-Period in Finder on a Mac.)
...
In this folder, incidentally,
are sample githook scripts for all the possible events you can "hook":
...
We're running our script before every commit ("pre-commit")
because otherwise all those libraries we failed to take out
will live on in your commit history, even if you delete them later.

### Writing a Bash Script

Now I don't really know bash, but that's what Google is for.
...
When I wrote this, I was using a company-issued Windows laptop
that had `Rscript.exe` in a non-standard location.
You likely have `Rscript` on [your PATH][36]
and can simplify accordingly.

In the next section,

* `HOOKS_DIR` is the folder the script is in:
  `.../your_repository/.git/hooks`.
...
We'll tell the R script what to _do_ with that string in the next section.
...
—is critical. Without it, the script would swap the library references,
delete all the right files,
and follow through with your original commit—_without staging its changes!_

`git add -u` ensures the changes stick.
But this line also has an important side effect, so beware:
_Any changes to your repo will be staged and committed,
even if you never staged them yourself._
...
—passes the repository's location to the R script.
We also need to tell the script what to do with it.

For that we use the [commandArgs][25] function.
...
your OS may need to know the file is _executable_.
If that's the case you'll get something like the yellow error message here:
...
Lastly, either for our future selves or for someone else using the script,
troubleshooting is always easier when you have good _messaging_.
...
If you do check out the enhanced script,
here's a quick preview of the differences you'll see in it:

* I don't load the whole tidyverse, only the specific packages I use.
* I provide a lot of messaging
  around which of those packages loaded successfully.
...
* The code is also more flexible, capturing any version of the libraries.
  If the user's R ecosystem upgrades to new library versions,
  I want the code to work but also message it.
...
[36]: https://astrobiomike.github.io/bash/modifying_your_path
