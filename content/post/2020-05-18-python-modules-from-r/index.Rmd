...
was that you could `import` arbitrary files from the same folder as the one you were in.
...
This is important to everything from file organization to workflow to troubleshooting installation issues.
I know how it works in R, for example:

* To use `library`, you must have installed the package using one of our canonical methods:
  `install.packages()`, or `remotes::install_github()`, for example.
* To use `source`, you refer to the file by its path relative to the working directory,
  or use an absolute reference, or preferably use Jenny Bryan's `here` package.
...
* The fact that there's `pip install`, and `conda install`, and virtual environments and everything,
...
* The fact that import is sort of a hybrid between library and source.
  See, when you run library package name,
