...
When you run `library(package_name)`,
all it does is to run all the R files in a particular directory of that package.

![_a partial list of R files run when you load `ggplot2`_](ggplot_r_directory.png)

In that sense, it's the same as downloading the package from GitHub and running this:
...
The process is different from sourcing scripts in two main ways:

1. One difference is that package authors are careful to write these files
   so that all they do is define functions.
   
   By contrast, when you source a script
   you might run a model, or print outputs or write them to file.
   
1. The other is that when you `library` the file,
   those functions don't show up in the upper right panel of RStudio,
   the way they would if you `source`d the file.
   They don't exist in what's called "the global environment."
   
   Instead, they get their own "namespace," their own partioned-off world
   where all function names are unique.
   You see namespaces in action when you call a function by its full name
   using double-colon notation: `package_name::function()`.
