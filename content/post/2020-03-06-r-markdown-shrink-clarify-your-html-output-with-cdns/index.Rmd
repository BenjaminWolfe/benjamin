---
title: "R Markdown: Shrink & Clarify Your HTML Output with CDNs"
...
Have you ever noticed, when you render an [R Markdown][4] file as HTML,
that it comes out as a single HTML file, easy to share with your readers?

If you haven't noticed, or you don't know what I mean, 
you can "follow along at home" with this [minimal reproducible example][3].

(I follow this same example through the entire post.
If you'd like to follow along but maybe don't have your laptop in front of you,
I put it up in a [GitHub repository][5],
and you can walk through the steps there.
For the first section, check [this commit][1] and [this one][2].)
...
at how much each of those affects the size of the file.
...
what's called _minified JavaScript_.
...
Now in your R script, that line relies on whoever's running your code
...
`report.html` _literally includes all of jQuery inside itself_.

But libraries are big, of course—sometimes really big.
So this code is what's called a [minified][6] version of the library.
...
Likewise, images are actually included _within the text of the HTML document_,
encoded as text.
To see this, scroll down in the source to where you'd expect to see the picture:

![](10-image-as-text.png)

You'll see that the picture is a png file, but it's encoded as a really long,
otherwise meaningless text string.
...
_diffing files_ just means comparing files (or contrasting them, technically)
...
It's what you see when you pull up a git UI and it shows you what's changed.

The problem is that most interfaces won't diff large files automatically,
and some won't diff them at all.
For example, this is what GitHub Desktop shows when I try to diff my HTML:

![](11-diff-too-large.png)

But even when you can diff the files, all the gobbledeegook—both minified code
and text-encoded images—makes it hard to make sense of the changes.

## Intermission!

To reduce file size and increase clarity while keeping things easy to share,
you'll need 3 tips:
...
In this case, we're now up to 916 kilobytes, another increase of 203 kB!
...
First, we address the _clarity_ of the code,
so that you can diff files and meaningfully keep track of changes.

You'll remember that the HTML file includes entire JavaScript libraries,
minified and embedded within them.
What we're going to do is _externalize_ those libraries
so the code and the content live in separate places.
...
We need to add to that so that `knitr` no longer tries to keep everything
...
Suddenly the HTML file is only 7 kB in size (down 99%!):

![](15-shrunk-html.png)

All of that content has been moved to a separate folder called `report_files`.
In a way, this is a wonderful thing.
If you look at the [diff from this commit][8] in my GitHub repository,
...
Personally I find this newfound clarity and readability refreshing.
I encourage you to check out [how clean the new HTML is][9],
either in my repo or on your own.
...
> If Boostrap is the most popular HTML, CSS, and JS library in the world,
...
You can see an example of this on [Bootstrap's site][10]:
...
Currently, your file does something similar.
It includes a reference to the Bootstrap library (along with other resources)—

![](20-local-library-references.png)

—but the references are _local_, pointing to files in the `report_files` folder.
What we need, then, is to _replace_ those references
with references to the Bootstrap CDN.
And of course we'll go for not just Bootstrap, but as many of them as possible.

We'll do this in three steps:

1. First we find all the libraries we're referencing,
   and see which ones have CDNs we can use.
1. Then we swap the local references to CDNs
   and remove the files that are no longer necessary.
1. Lastly we'll automate the whole thing,
...
This step might be different for your scenario.
...
and second so you can follow it for your own report.

So what are all the libraries we're using?
...
Let's notice a few things about what we're looking at:

* Each of these is either a _link_ (for a CSS file)
  or a _script_ (for JavaScript).
* The part we want to change is in quotes after `href=` or `src=`.
* Each time it starts with `report_files/`,
  since that's where all the local libraries are stored.
* Each of them has [semantic versioning][11] (or _semver_): generally x.y.z,
  where x is the major version, y the minor version, and z the current patch.
...
libraries %>% 
  knitr::kable(col.names = c("Filepath w/in report_files", "Version")) %>% 
  row_spec(c(1:5, 7:8, 12, 14), color = "black", background = "#c2ffca")
```

I was able to find CDNs for the 9 _green_ rows,
simply by googling _[name of library] [version] CDN_ for each one.

![](21-google-jquery.png)

Here, in order, are the ones I found.
While I tried to match the versioning,
if it was close sometimes I took what I could get.
...
We simply have to replace the strings that look like this—
...
This is a great opportunity to take a look behind the scenes.
Remember what the report looked like before you deleted that href:

![](24-with-bootstrap.png)

If you save the report right now, with the href deleted out,
and open it in Chrome, it looks like this instead:

![](25-without-bootstrap.png)

Seeing this helps you understand what Bootstrap was doing for our report,
and it also helps verify when we fix it.
Now copy-paste in the URL of the CDN—
...
You can copy-paste them into your file if you're following along at home.
...
we only deleted _that one file_, but we left the rest standing.
...
but it's easiest for me to consolidate the important files to a temp directory,
...
I want to take a moment to celebrate our successes.
...
and we truly no longer needed the local files!
...
feel free to skip down to _Zipping Your Files_ and call it good!
...
Note: If you're not familiar with the `here` package,
I highly recommend [Jenny Bryan][13]'s ode,
...
Here [purrr::reduce2][15] is the work horse.
It runs through our replacements iteratively, as if we'd written it like this:
...
and this function will do that for us:
...
I turn to the [fs package][18].
It has intuitively named, vectorized functions like

* [path][19] to construct a filepath,
...
Now that we've scripted the whole process,
I'd prefer not to have to think about it at all.

We can accomplish that goal by creating what's called a [git hook][23].
