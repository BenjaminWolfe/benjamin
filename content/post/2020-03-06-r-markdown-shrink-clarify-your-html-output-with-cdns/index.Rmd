...
Suddenly the HTML file is only 7 kB (down 99%!):
...
Currently, your file does something similar, not quite the same;
it doesn't use CDNs.
It does point to Bootstrap and other libraries at the top—

![](20-local-library-references.png)

—but the library references are _local_,
pointing to files in the `report_files` folder.
What we want, then, is to _replace_ those references with CDNs.

Then we can delete the local versions we no longer need, and—back to
the problem we were solving—we'll have reduced the overall filesize.
